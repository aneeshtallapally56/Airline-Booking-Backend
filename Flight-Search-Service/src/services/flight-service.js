...
async function getAllFlights(query){
    ...
    // Fix: Check for empty string differently
    ...
    if(query.trips) {
        ...
        // Add validation for undefined values after split
        if(!departureAirportId || !arrivalAirportId) {
            throw new AppError(
                'Invalid trip format. Expected format: departureAirportId-arrivalAirportId',
                StatusCodes.BAD_REQUEST
    ...
    try {
        ...
        console.error('Error in getAllFlights:', error); // Add logging to see the actual error
        throw new AppError(
            'Something went wrong while getting all flights',
            StatusCodes.INTERNAL_SERVER_ERROR
        );  
